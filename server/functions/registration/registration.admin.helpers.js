...
const updateTodaysNewUserCount = async () => {
  const admin = await Admin.findOne({ id: "main" });
  let mostRecentDailyRegisteredUsersObj =
    admin.total_new_users_daily[admin.total_new_users_daily.length - 1];
...
const dailyUserRegistrationUpdate = async () => {
  ...
  let mostRecentDailyRegisteredUsersObjID =
    admin.total_new_users_weekly[admin.total_new_users_weekly.length - 1]._id;
  let dailyRegistrationCount;
  let dailyRegistrationArray = admin.total_new_users_daily;
  let currentWeeklyRegistrationCount = admin.total_new_users_weekly.id(
    mostRecentDailyRegisteredUsersObjID
  ).new_users_this_week;

  currentWeeklyRegistrationCount =
    currentWeeklyRegistrationCount + dailyRegistrationCount;
...
const weeklyUserRegistrationUpdate = async () => {
  ...
  admin.total_new_users_daily[admin.total_new_users_daily.length - 1].save({
