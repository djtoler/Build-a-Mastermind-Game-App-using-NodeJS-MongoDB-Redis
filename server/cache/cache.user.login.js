...
const { validateVisitorLoginInput} = require("../functions/login/login.functions");
const { authorizeUserToStartGame } = require("../functions/login/login.functions");
const { loginValidationHelpers } = require("../functions/login/login.helpers");
...
const findUserFromLoginInCache = async (email, password) => {
  ...
  if (runUserValidation.isValidLoginInput == false) {
    return;
  }
  ...
  if (authorizeUser.user) {
    const setUserInCache = await cacheEngine.set(
      email,
      JSON.stringify(authorizeUser.user)
    );
  }
